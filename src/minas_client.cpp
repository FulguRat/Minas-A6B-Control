...
{
    // 6081h, unit: pulse, range: 0 - 4294967295, U32
    uint32_t u32val = (uint32_t)val;
    manager_.writeSDO<uint32_t>(slave_no_, 0x6081, 0x00, u32val);
}

void MinasClient::setProfileAcceleration(uint32_t val)
{
    // 6083h, unit: pulse/s^2, range: 0 - 4294967295, U32
    uint32_t u32val = (uint32_t)val;
    manager_.writeSDO<uint32_t>(slave_no_, 0x6083, 0x00, u32val);
}

void MinasClient::setProfileDeceleration(uint32_t val)
{
    // 6084h, unit: pulse/s^2, range: 0 - 4294967295, U32
    uint32_t u32val = (uint32_t)val;
